...
def atomic_replace_with_retry(src, dst, max_retries=20, retry_delay=0.05):
    ...
    print(f"[PROPORTIONAL] warning: could not replace {dst}: {last_err}")
    try:
        os.remove(src)
    except Exception:
        pass
...
def write_action(action):
    ...
    return atomic_replace_with_retry(tmp, ACTION_FILE)
...
def save_log():
    ...
    if log_records:
        ...
        try:
            ...
            saved = len(log_records)
            log_records = []
            _log_initialized = True
            print(f"[PROPORTIONAL] Log saved ({saved} new steps)")
        except PermissionError as exc:
            print(f"[PROPORTIONAL] warning: log save postponed ({exc})")
    ...
    try:
        while True:
            ...
            if not action_written:
                print("[PROPORTIONAL] action write failed; waiting for MATLAB timeout")
                continue
